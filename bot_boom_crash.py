...
#  ⚠️ Ajustado para saldo de ~$4.00
# ══════════════════════════════════════════
META_DIARIA_USD = 3.00        # Meta conservadora (~75% del saldo)
STOP_LOSS_DIARIO_USD = -2.00  # Máximo a perder por día (~50% del saldo)

# Configuración del Multiplicador
STAKE_BASE = 0.50             # Reducido de $1.00 a $0.50
MULTIPLIER = 100
TAKE_PROFIT_TRADE = 1.50      # TP proporcional al nuevo stake
STOP_LOSS_TRADE = 0.50        # SL = stake completo (reactivado)
...
# ══════════════════════════════════════════
#  CANDADO GLOBAL (1 solo trade a la vez)
# ══════════════════════════════════════════
    ...
    await ws.send(json.dumps({
        ...
        "parameters": {
            ...
            "limit_order": {
                ...
                "stop_loss": STOP_LOSS_TRADE   # ✅ SL reactivado
            }
        }
    }))
    ...
    while True:
        try:
            async with websockets.connect(url) as ws:
                ...
                while True:
                    # ✅ FIX: renombrado a 'raw_msg' para no pisar la variable 'msg' usada abajo
                    ...
                    if "authorize" in raw_msg:
                        ...
                        enviar_telegram(
                            f"⚡ Bot Boom/Crash V2 (Conservador)\n"
                            f"💰 Saldo: ${balance_actual} {moneda_cuenta}\n"
                            f"⚙️ Stake: ${STAKE_BASE} | Mult: x{MULTIPLIER} | TP: ${TAKE_PROFIT_TRADE} | SL: ${STOP_LOSS_TRADE}\n"
                            f"🛡️ 1 trade global a la vez | SL diario: ${abs(STOP_LOSS_DIARIO_USD)}"
                        )
                        for sym in SIMBOLOS:
                    ...
                    if "tick" in raw_msg and not bot_pausado_por_hoy:
                        ...
                        if len(est["prices"]) > 1:
                            ...
                            # ✅ Disparo solo si NO hay ningún trade abierto (candado global)
                    ...
                    if "proposal_open_contract" in raw_msg:
                        ...
                        if contract.get("is_sold"):
                            ...
                            # Buscamos el ctx en el símbolo correcto
                            ctx = {}
                            if symbol in estado_simbolos:
                                ctx = estado_simbolos[symbol].get("ultimo_ctx", {})
                            ...
                            if profit > 0:
                                ...
                                trades_perdidos += 1
                                msg_res = f"❌ Trade cerrado (Pérdida) -${abs(round(profit,2))}"
                            ...
                            resumen = (
                                ...
                                f"💵 Saldo Real: ${balance_actual} {moneda_cuenta}\n"
