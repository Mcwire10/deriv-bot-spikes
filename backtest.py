"""
backtest.py — EUR/USD edge search
==================================
DIAGNÓSTICO: EUR/USD falla con trailing porque ATR M15 ~8 pips.
El Stage2 trail pone el SL más lejos que el move total → todo cierra en BE.
...
async def fetch_candles(granularity: int, months: int) -> list:
    ...
    label       = {300: "M5", 900: "M15", 3600: "H1"}.get(granularity, f"{granularity}s")

    print(f"[fetch] {SYMBOL} {label} | ~{need} velas | {batches} batch(es)")
    ...
    for c in all_c:
        if c["epoch"] not in seen:
            seen.add(c["epoch"])
            unique.append(c)
    print(f"[fetch] {label} — {len(unique)} velas unicas\n")
    return unique
...
def in_session(epoch, h_start, h_end):
    return h_start <= datetime.fromtimestamp(epoch, tz=timezone.utc).hour < h_end

# ─────────────────────────────────────────────
# SIMULACIÓN — TP FIJO (sin trailing)
# EUR/USD: moves muy pequeños, el trailing los corta siempre
# ─────────────────────────────────────────────
def simulate_fixed_tp(candles_after, direction, entry, sl, tp):
    for i, c in enumerate(candles_after[:300]):
        hit_sl = c["low"] <= sl if direction == "LONG" else c["high"] >= sl
        hit_tp = c["high"] >= tp if direction == "LONG" else c["low"] <= tp

        if hit_tp and hit_sl:
            # Ambas en la misma vela — heurística: vela a favor → TP primero
            tp_first = (c["close"] >= c["open"]) if direction == "LONG" else (c["close"] <= c["open"])
            exit_p = tp if tp_first else sl
        elif hit_tp:
            exit_p = tp
        elif hit_sl:
            exit_p = sl
    ...
    ep     = candles_after[min(299, len(candles_after)-1)]["close"]
    delta  = (ep - entry) / entry * (1 if direction == "LONG" else -1)
    profit = round(max(STAKE * MULTIPLIER * delta, -STAKE), 2)
    sl_d   = abs(entry - sl)
    r      = (ep - entry) / sl_d if direction == "LONG" else (entry - ep) / sl_d
    return {"profit": profit, "r": round(r, 2), "candles": 300, "reason": "timeout"}
...
def strat_london_breakout(candles_m5, idx):
    ...
    asia      = [c for c in candles_m5[:idx] if day_ts <= c["epoch"] < london_ts]

    if len(asia) < 20: return None
    ...
    if asia_range < atr * 0.4 or asia_range > atr * 4.0: return None

    curr   = ultima["close"]
    buffer = asia_range * 0.08

    if curr > asia_high + buffer:
        return {"direction": "LONG",  "entry": curr,
                "sl_price": round(asia_high - buffer, 5),
                "tp_price": round(curr + asia_range * 1.5, 5),
                "detail": f"Breakout alcista | Asia {round(asia_range*10000,1)}p"}

    if curr < asia_low - buffer:
        return {"direction": "SHORT", "entry": curr,
                "sl_price": round(asia_low + buffer, 5),
                "tp_price": round(curr - asia_range * 1.5, 5),
                "detail": f"Breakout bajista | Asia {round(asia_range*10000,1)}p"}

    return None

# ─────────────────────────────────────────────
# ESTRATEGIA C — RSI DIVERGENCIAS H1 + TP FIJO
# H1 filtra el ruido que destruye señales en M15
# ─────────────────────────────────────────────
    ...
    for p1 in range(n - max_pts - 2, n - 3):
        ...
        for p2 in range(p1 + 2, min(p1 + max_pts + 1, n - 1)):
            ...
            if lows[p2] < lows[p1] and p2_rsi > p1_rsi and p2 == n - 2:
                ...
                return {"direction": "LONG", "entry": ultima["close"],
                        "sl_price": sl, "tp_price": tp,
                        "detail": f"RSI Div Bull H1 | {round(p1_rsi,1)}→{round(p2_rsi,1)}"}
    ...
    for p1 in range(n - max_pts - 2, n - 3):
        ...
        for p2 in range(p1 + 2, min(p1 + max_pts + 1, n - 1)):
            ...
            if highs[p2] > highs[p1] and p2_rsi < p1_rsi and p2 == n - 2:
                ...
                tp   = round(ultima["close"] - dist * 2.0, 5)
                return {"direction": "SHORT", "entry": ultima["close"],
                        "sl_price": sl, "tp_price": tp,
                        "detail": f"RSI Div Bear H1 | {round(p1_rsi,1)}→{round(p2_rsi,1)}"}
...
# BACKTEST GENÉRICO
# ─────────────────────────────────────────────
async def run_backtest(name, candles, signal_fn, session, cooldown_mult=3, is_breakout=False):
    ...
    for i in range(warmup, len(candles) - 1):
        ...
        signal = signal_fn(candles, i) if is_breakout else signal_fn(candles[:i+1])
        ...
        print(f"  {'✅' if result['profit'] > 0 else '❌'} {dt.strftime('%m-%d %H:%M')} "
              f"| {signal['direction']} | ${result['profit']} | {result['r']}R | {result['reason']}")
...
def analyze(name, trades):
    ...
    return {
        "name": name, "total": total, "wr": wr, "profit": profit,
        "avg_win": avg_w, "avg_loss": avg_l, "expectancy": exp,
        "max_dd": max_dd, "pf": pf, "tp_pct": tp_pct,
...
async def main(months=6):
    ...
    print(f"  A) Bollinger MR M15  B) London Breakout M5  C) RSI Div H1")
    print(f"{'='*55}\n")
    ...
    print("Descargando M15..."); c_m15 = await fetch_candles(900,  months)
    print("Descargando M5...");  c_m5  = await fetch_candles(300,  months)
    print("Descargando H1...");  c_h1  = await fetch_candles(3600, months)

    results = []

    print("─── A) Bollinger Mean Reversion M15 (Londres 08-12) ───")
    results.append(analyze("A) Bollinger Mean Rev M15",
        await run_backtest("Bollinger", c_m15, strat_bollinger, (8, 12))))

    print("\n─── B) London Breakout M5 (08:00-08:45) ───")
    results.append(analyze("B) London Breakout M5",
        await run_backtest("Breakout", c_m5, strat_london_breakout, (8, 9),
                           cooldown_mult=6, is_breakout=True)))

    print("\n─── C) RSI Divergencias H1 (overlap 10-14) ───")
    results.append(analyze("C) RSI Div H1 TP fijo",
        await run_backtest("RSI_H1", c_h1, strat_rsi_div_h1, (10, 14))))

    # Consola
    print(f"\n{'='*55}")
    for s in results:
        if s: print(f"  {s['name']}: {s['total']}t | WR {s['wr']}% | PF {s['pf']}x | ${s['profit']} | {verdict(s)}")
        else: print("  Sin trades")
    ...
    if winner:
        v = verdict(winner)
        concl = (f"✅ Agregar al scanner con _{winner['name']}_"
                 if v != "❌ SIN EDGE"
                 else "❌ EUR/USD queda fuera del scanner por ahora")
        await send_telegram(
            f"*VEREDICTO EUR/USD*\n══════════════════════════\n"
            f"Mejor: *{winner['name']}*\n"
            f"PF {winner['pf']}x | WR {winner['wr']}% | ${winner['profit']}\n"
            f"──────────────────────\n{concl}"
        )
